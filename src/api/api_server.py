#!/usr/bin/env python3
"""
Edgefit-Coach FastAPI Server
===========================

Comprehensive backend API server providing real-time video streaming, dashboard data,
AI chat interface, and posture analysis endpoints. Built with FastAPI for high-performance
asynchronous operations and WebSocket support.

Key Features:
    - Real-time video streaming with pose overlay
    - WebSocket-based motivation quote delivery
    - Dashboard data aggregation and analytics
    - AI-powered chat interface for posture coaching
    - Automated posture report generation and analysis
...
API Endpoints:
    Video Streaming:
        ...
        - GET /video/stream: Live video feed
        - POST /video/page-control: Control based on page
    
    Dashboard & Analytics:
        - GET /dashboard/data: Get posture analytics
        - POST /dashboard/refresh: Refresh metrics
    
    AI Chat Interface:
        - POST /chat/message: Send message to AI coach
        - GET /chat/history: Get conversation history
        - DELETE /chat/history: Clear chat history
    
    Analysis & Reports:
        - POST /analyze/report: Generate AI analysis
        - GET /analyze/report-file: Download report
    ...
    WebSocket:
        - WS /ws/motivation: Real-time motivation quotes

Dependencies:
    - FastAPI: High-performance web framework
    - OpenCV: Video processing and streaming
    - Uvicorn: ASGI server for production deployment
    - Custom modules: llm_handler, posture_db, dashboard_viewer
...
from utils.posture_db import PostureConverter
from utils.dashboard_viewer import PostureDashboard
from utils.video_streamer_pose import start_pose_streaming, stop_pose_streaming, is_pose_streaming, get_pose_stream_url, get_pose_frame_generator
...
def start_streamlit_frontend():
    ...
    try:
        streamlit_process = subprocess.Popen(
            ["python", "-m", "streamlit", "run", "frontend/frontend_test.py", "--server.port", "8501", "--server.headless", "true"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
            encoding='utf-8',
...
def generate_video_frames():
    ...
    while True:
        try:
            ...
            frame_file = os.path.join(os.path.dirname(__file__), "..", "..", "temp", "current_frame.jpg")
...
app.mount("/hls", StaticFiles(directory=PROCESSED_DIR), name="hls")

@app.get("/hls/stream.m3u8")
async def get_hls_playlist():
    """Serve HLS playlist file."""
    playlist_path = os.path.join(PROCESSED_DIR, "stream.m3u8")
...
async def get_hls_segment(filename: str):
    ...
    segment_path = os.path.join(PROCESSED_DIR, filename)
...
async def get_dashboard_data():
    ...
    try:
        ...
        endpoint_values_file = os.path.join(TEMP_DIR, "endpoint_values.json")
...
async def analyze_posture_report(request: AnalysisRequest = None):
    ...
    try:
        ...
        report_file = os.path.join(TEMP_DIR, "posture_report.txt")
...
async def check_file_status():
    ...
    files_to_check = [
        os.path.join(DATA_DIR, "posture_summary.json"),
        os.path.join(TEMP_DIR, "endpoint_values.json"),
        os.path.join(TEMP_DIR, "posture_report.txt"),
        os.path.join(DATA_DIR, "chat_history.json"),
        os.path.join(DATA_DIR, "motivation_quotes.json")
    ]
