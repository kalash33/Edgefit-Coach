#!/usr/bin/env python3
"""
Edgefit-Coach: Enhanced Headless Posture Monitoring with Desktop Notifications
============================================================================

This module provides advanced real-time posture monitoring with AI coaching,
desktop notifications, and comprehensive analytics. It operates in headless mode
for optimal performance while providing rich feedback through notifications.

Key Features:
    - Real-time posture detection using MediaPipe
    - Cross-platform desktop notifications (Windows/Linux)
    - AI-powered motivational coaching
    - Stretching exercise detection
    - Discrete 30-second window logging
    - ONNX model optimization support
    - Performance analytics and reporting

Usage:
    python main_webcam_headless1.py [--headless]

Dependencies:
    - OpenCV (cv2): Video capture and image processing
    - MediaPipe: Pose detection and landmark extraction
    - win10toast/notify2: Desktop notifications
    - onnxruntime: Optimized model inference (optional)
    - Custom modules: llm_handler for AI communication

Author: Edgefit-Coach Team
Version: 1.0.0
License: MIT
"""
...
from utils.llm_handler import ask_llm, is_error_response
...
def show_desktop_notification(title: str, message: str, icon: str = "dialog-information", urgency: str = "normal") -> None:
    """
    ...
    notification system (Windows Toast or Linux notify2).
    
    Args:
        title (str): Notification title/header text
        message (str): Main notification message content
        icon (str): Icon identifier for Linux notifications (default: "dialog-information")
        urgency (str): Notification urgency level - "low", "normal", or "critical"
                      Affects display duration and priority
    
    Returns:
        None
        
    Note:
        - Windows: Uses win10toast with 8-10 second duration
        - Linux: Uses notify2 with D-Bus integration
        - Gracefully handles missing notification libraries
        - Runs in threaded mode to avoid blocking main execution
    
    Example:
        show_desktop_notification(
            "Posture Alert", 
            "Good posture: 85% - Keep it up!",
            urgency="normal"
        )
    """
...
def detect_overhead_reach(lmList: List[List[int]]) -> bool:
    """
    Detect overhead reach stretching exercise.
    
    Identifies when both arms are raised straight up above the head,
    similar to reaching for the ceiling. Uses 6-point validation system
    for accurate detection.
    
    Args:
        lmList (List[List[int]]): MediaPipe pose landmarks in format [id, x, y]
                                 Requires at least 17 landmarks (0-16)
    
    Returns:
        bool: True if overhead reach is detected, False otherwise
        
    Validation Criteria:
        1. Both wrists 150px above shoulders
        2. Arms symmetric (height difference < 20px)
        3. Both arms centered over body
        4. Both elbows raised above shoulder level
        5. Proper arm alignment and positioning
...
class PostureLogger:
    ...
    def __init__(self, log_interval=3.0, window_duration=30.0, summary_file=None, motivation_file=None):
        # Get project root directory (go up from src/core to project root)
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        
        # Set default paths if not provided
        if summary_file is None:
            summary_file = os.path.join(project_root, 'data', 'posture_summary.json')
        if motivation_file is None:
    ...
    def _generate_motivation_quote(self):
        ...
        try:
            ...
            if not is_error_response(response):
                ...
                # Show desktop notification with performance info
                performance_text = f"Good: {self.good_posture_count} | Slouch: {self.slouching_count} | Performance: {good_percentage:.1f}%"
                notification_title = "🤖 AI Posture Coach"
                notification_message = f"{response.strip()}\n\n📊 {performance_text}"
                
                # Determine urgency and icon based on performance
                if good_percentage < 40:
                    urgency = "critical"
                    icon = "dialog-warning"
                ...
                show_desktop_notification(notification_title, notification_message, icon, urgency)
            else:
...
def save_frame_for_web_stream(frame):
    """Save current frame for web streaming."""
    try:
        # Get project root directory and ensure temp directory exists
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        temp_dir = os.path.join(project_root, 'temp')
        os.makedirs(temp_dir, exist_ok=True)
        
        # Save frame as JPEG for web streaming
        cv2.imwrite(os.path.join(temp_dir, "current_frame.jpg"), frame)
        
        # Also save frame data as pickle for API server
        with open(os.path.join(temp_dir, "current_frame.pkl"), "wb") as f:
            pickle.dump(frame, f)
    except Exception as e:
        pass  # Silently fail to avoid disrupting main loop
...
def main():
    ...
    # Check if running in headless mode (from API) or with GUI
    headless_mode = len(sys.argv) > 1 and sys.argv[1] == "--headless"
    web_stream_mode = len(sys.argv) > 2 and sys.argv[2] == "--web-stream"
    
    if headless_mode and web_stream_mode:
        print("🎥 Starting Real-time Pose Detection with Webcam (Web Stream Mode)")
    elif headless_mode:
        print("🎥 Starting Real-time Pose Detection with Webcam (Headless Mode)")
    else:
    ...
    # Test camera read capability
    print("🔍 Testing camera read capability...")
    test_ret, test_frame = cap.read()
    if not test_ret:
        print("❌ Error: Camera opened but cannot read frames!")
        print("💡 Try: Check Windows camera privacy settings")
        cap.release()
        return
    else:
    ...
    # Initialize posture logger (paths will be set automatically to absolute paths)
    ...
    if headless_mode:
        print("📋 Headless Mode - No GUI Controls:")
        print("   - Send SIGTERM or SIGINT to quit")
        print("   - Running in background mode")
        print("   - All detection and logging active")
    else:
        print("📋 Controls:")
    ...
    try:
        while running:
            ...
            # Add UI elements (only in GUI mode)
            ...
            # Report FPS every 30 frames (headless mode only)
            if headless_mode and frame_count % 30 == 0:
                elapsed_time = cTime - start_time
                avg_fps = frame_count / elapsed_time if elapsed_time > 0 else 0
                print(f"📹 FPS: {fps:.1f} | Avg: {avg_fps:.1f} | Frames: {frame_count}")
                start_time = cTime
                frame_count = 0
            ...
            if posture_detection_enabled and len(lmList) >= 13:  # Ensure we have all required landmarks
                ...
                if logging_enabled and posture_logger.should_log_posture():
                    ...
                    # Console output for headless monitoring
                    if headless_mode:
                ...
                # Display continuous posture status (GUI mode)
                if not headless_mode:
            ...
            if len(lmList) >= 17:  # Ensure we have all required landmarks for stretching
                ...
                if stretch_detected:
                    ...
                    # Display stretching status (GUI mode)
                    if not headless_mode:
            ...
            if web_stream_mode:
                save_frame_for_web_stream(img)
            
            # Display the image (GUI mode only)
            if not headless_mode:
