...
class PostureDashboard:
    ...
    def __init__(self, posture_file: str = None, report_file: str = None):
        # Get project root directory (go up from src/utils to project root)
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        
        # Set default paths if not provided
        if posture_file is None:
            posture_file = os.path.join(project_root, 'data', 'posture_summary.json')
        if report_file is None:
            report_file = os.path.join(project_root, 'temp', 'posture_report.txt')
        """
